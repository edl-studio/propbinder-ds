import { Component, ViewEncapsulation, input, output, computed, signal, effect } from '@angular/core';
          ...
          <div class="ds-data-table__column-panel-content">
            @for (column of hidableColumns(); track column.id) {
              <label class="ds-data-table__column-toggle">
                ...
                />
                <span class="ui-xs-regular tw-text-default-tertiary">{{ getColumnLabel(column) }}</span>
              </label>
            }
          </div>
        </div>
                ...
                <tr class="ds-data-table__tr">
                  @for (header of headerGroup.headers; track header.id) {
                    <th 
                      ...
                      [class.ds-data-table__th--truncate]="shouldTruncateColumn(header.column)"
                      ...
                      @if (!header.isPlaceholder) {
                        @if (header.column.getCanSort()) {
                          <button 
                            [class]="header.column.getIsSorted() ? 'ds-data-table__sort-button ds-data-table__sort-button--active' : 'ds-data-table__sort-button'"
                ...
                <tr 
                  ...
                  @for (cell of row.getVisibleCells(); track cell.id) {
                    <td 
                      ...
                      [class.ds-data-table__td--truncate]="shouldTruncateColumn(cell.column)"
          ...
          <div class="ds-data-table__footer tw-text-default-secondary">
            <div class="ds-data-table__info">
              <span class="body-xs-regular">
                ...
                to {{ Math.min((table().getState().pagination.pageIndex + 1) * table().getState().pagination.pageSize, effectiveRowCount()) }}
            ...
            <div class="ds-data-table__pagination">
              ...
              <div class="ds-data-table__pagination-buttons">
                ...
                <span class="body-xs-regular">
                  Page {{ table().getState().pagination.pageIndex + 1 }} of {{ effectivePageCount() }}
                </span>
...
export class DsDataTableComponent<T = any> {
  ...
  table = createAngularTable<T>(() => ({
    ...
    getPaginationRowModel: this.paginated() && !this.serverSide() ? getPaginationRowModel() : undefined,
