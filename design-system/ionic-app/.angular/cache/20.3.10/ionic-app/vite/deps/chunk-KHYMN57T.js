...
} from "./chunk-53KHQLJJ.js";
import {
  isPlatformBrowser,
  isPlatformServer
} from "./chunk-ILS3C6C2.js";
import {
  APP_ID,
  BehaviorSubject,
  CSP_NONCE,
...
// node_modules/ng-primitives/fesm2022/ng-primitives-state.mjs
...
function createStateInjector(token, options = {}) {
  return (injectOptions = {}) => {
    ...
    if (options.deferred) {
      return computed(() => value && Object.keys(value() ?? {}).length === 0 ? void 0 : value?.());
    }
...
function createControlledInput(property) {
  ...
  const inputDefinition = symbol ? property[symbol] : void 0;
...
var _NgpExitAnimationManager = class _NgpExitAnimationManager {
  constructor() {
    this.instances = [];
  }
  ...
  exit() {
    return __async(this, null, function* () {
      yield Promise.all(this.instances.map((instance) => instance.exit()));
    });
  }
};
_NgpExitAnimationManager.ɵfac = function NgpExitAnimationManager_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _NgpExitAnimationManager)();
};
_NgpExitAnimationManager.ɵprov = ɵɵdefineInjectable({
  token: _NgpExitAnimationManager,
  factory: _NgpExitAnimationManager.ɵfac
});
var NgpExitAnimationManager = _NgpExitAnimationManager;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(NgpExitAnimationManager, [{
    type: Injectable
  }], null, null);
})();
...
_NgpExitAnimation.ɵfac = function NgpExitAnimation_Factory(__ngFactoryType__) {
  return new (__ngFactoryType__ || _NgpExitAnimation)();
};
_NgpExitAnimation.ɵdir = ɵɵdefineDirective({
  type: _NgpExitAnimation,
  selectors: [["", "ngpExitAnimation", ""]],
  exportAs: ["ngpExitAnimation"]
});
var NgpExitAnimation = _NgpExitAnimation;
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(NgpExitAnimation, [{
    type: Directive,
    args: [{
    ...
    exit: () => {
      return new Promise((resolve, reject) => {
        ...
        if (animations.length > 0) {
          Promise.all(animations.map((anim) => anim.finished)).then(() => resolve()).catch((err) => {
...
function setupFocus({
  ...
  focusMonitor.monitor(elementRef, focusWithin).pipe(safeTakeUntilDestroyed()).subscribe((focusOrigin) => {
    if (disabled()) {
      return;
    }
...
function setupFocusVisible({
  ...
  const focusMonitor = inject(FocusMonitor);
  const isFocused = signal(false);
  focusMonitor.monitor(elementRef.nativeElement).pipe(safeTakeUntilDestroyed()).subscribe((origin) => (
...
function fromResizeEvent(element, {
  ...
  return new Observable((observable) => {
    ...
    explicitEffect([disabled], () => setupOrTeardownObserver(), {
      injector
    });
...
function observeResize(elementFn) {
  ...
  effect(() => {
    ...
    untracked(() => {
      ...
      subscription?.unsubscribe();
      subscription = fromResizeEvent(targetElement, {
        injector
      }).pipe(safeTakeUntilDestroyed(destroyRef)).subscribe((event) => dimensions.set({
        width: event.width,
        height: event.height
      }));
    });
...
function injectDimensions() {
  ...
  fromResizeEvent(elementRef.nativeElement).pipe(safeTakeUntilDestroyed(destroyRef), map(({
    width,
    height
  }) => ({
    width,
...
function onDomRemoval(element, callback) {
  ...
  fromResizeEvent(element).pipe(safeTakeUntilDestroyed()).subscribe((dimensions) => {
...
function fromMutationObserver(element, {
  childList,
  subtree,
  attributes,
  characterData,
...
function setupOverflowListener(element, {
  ...
  merge(fromResizeEvent(element, {
    ...
    injector,
    characterData: true
  })).pipe(safeTakeUntilDestroyed(destroyRef)).subscribe(() => hasOverflow.set(element.scrollWidth > element.clientWidth || element.scrollHeight > element.clientHeight));
  return hasOverflow;
}
...
//# sourceMappingURL=chunk-KHYMN57T.js.map
